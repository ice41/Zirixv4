...
server = {}
job = {}
Tunnel.bindInterface("zElectrician",server)
Tunnel.bindInterface("zElectrician",job)
...
RegisterServerEvent('zElectrician:roupa')
AddEventHandler('zElectrician:roupa', function()
	local source = source
	local user_id = vRP.getUserId(source)
    if user_id then
