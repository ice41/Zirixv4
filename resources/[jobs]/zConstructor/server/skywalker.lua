...
server = {}
job = {}
Tunnel.bindInterface("zConstructor",server)
Tunnel.bindInterface("zConstructor",job)
	...
	if user_id then
		if ferramenta[user_id] == 0 or not ferramenta[user_id] then
			if vRP.tryGetInventoryItem(user_id,config.item,quantidade[source]) then
				...
				TriggerClientEvent("Notify",source,"negado","Você precisa de <b>"..quantidade[source].."x Ferramentas</b>.",8000)
			end
		end
	end
	return false
