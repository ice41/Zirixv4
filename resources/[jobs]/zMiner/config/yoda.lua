config = {}

--[ AMMOUNT ]----------------------------------------------------------------------------------------------------------------------------

-- QUANTIDADE DE MINERIOS QUE IRA VIM MATH.RANDOM QUANTIDADE ALEATORIO ENTRE 1 E 3.

config.miner = { 1 , 3 }

--[ MININGS ]----------------------------------------------------------------------------------------------------------------------------

-- MINERIOS QUE IRAM VIM NA MINERAÇÃO.

config.ferro = "minerio-ferro"
config.prata = "minerio-prata"
config.ouro = "minerio-ouro"
config.diamante = "minerio-diamante"

--[ LASTVEH ]----------------------------------------------------------------------------------------------------------------------------

-- VEICULO NECESSARIO PARA INICIAR A ROTA, E NECESSARIO A HASH

config.lastveh = 48339065

--[ ITEM ]----------------------------------------------------------------------------------------------------------------------------

-- ITEM NECESSARIO PARA MINERAR E A QUANTIDADE QUE SERA USADA.

config.stone = "ponta-britadeira"
config.ammountstone = 1

--[ ROCKS ]----------------------------------------------------------------------------------------------------------------------------

-- ROCHAS PARA MINERAR.

config.rocks = {
	[1] = { ['x'] = 2925.86, ['y'] = 2792.39, ['z'] = 41.28 },
	[2] = { ['x'] = 2927.92, ['y'] = 2788.92, ['z'] = 40.63 },
	[3] = { ['x'] = 2934.52, ['y'] = 2784.21, ['z'] = 40.17 },
	[4] = { ['x'] = 2938.35, ['y'] = 2774.26, ['z'] = 39.76 },
	[5] = { ['x'] = 2938.97, ['y'] = 2768.87, ['z'] = 39.72 },
	[6] = { ['x'] = 2952.42, ['y'] = 2767.91, ['z'] = 40.02 },
	[7] = { ['x'] = 2957.69, ['y'] = 2772.68, ['z'] = 40.32 },
	[8] = { ['x'] = 2972.52, ['y'] = 2775.06, ['z'] = 39.32 },
	[9] = { ['x'] = 2983.28, ['y'] = 2763.07, ['z'] = 43.64 },
	[10] = { ['x'] = 2991.06, ['y'] = 2776.44, ['z'] = 43.75 },
	[11] = { ['x'] = 2993.86, ['y'] = 2753.04, ['z'] = 43.68 },
	[12] = { ['x'] = 2956.27, ['y'] = 2820.02, ['z'] = 43.11 },
	[13] = { ['x'] = 2944.35, ['y'] = 2818.70, ['z'] = 43.53 }
}