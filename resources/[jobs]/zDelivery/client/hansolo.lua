...
Tunnel.getInterface("zGarages", zSERVER)
Tunnel.bindInterface("zDelivery", delivery)
Proxy.addInterface("zDelivery", delivery)
delivery = Tunnel.getInterface("zDelivery")

local x,y,z = 5.51,-159.69,56.21
...
function pertoCentral()
    ...
    local distance = GetDistanceBetweenCoords(x,y,z, localPlayer.x, localPlayer.y, localPlayer.z, true)
    if (distance<=36) then
      	return true
    end
end
...
function CriaBlipRestaurante(restaurantes,destino,blipCor)
	...
	AddTextComponentString("Pedido Delivey")
...
function criaMoto()
	...
	if not nveh then
		...
		nveh = CreateVehicle(mhash,77.9, 265.69, 109.28, 159.15+0.5,0.86,true,false)
		SetVehicleIsStolen(nveh,false)
		SetVehicleOnGroundProperly(nveh)
		SetEntityInvincible(nveh,false)
		SetVehicleNumberPlateText(nveh,"zirix")
			...
			if trabalhando and pedido then
				...
				if distance <= 50 then
					...
					if IsControlJustPressed(0,101) then
						TriggerEvent("Notify","sucesso", "Aguarde enquanto preparam o pedido para entrega",4000)
							RemoveBlip(blip)  -- remove o blip anterior
							Citizen.Wait(4000)  
							TriggerEvent("Notify","sucesso","Pedido recebido, entregue ao cliente marcado em seu GPS",8000)
			...
			if trabalhando and entrega then
				...
				if distance <= 50 then
					...
					if distance < 3 then
						...
						if IsControlJustPressed(0,101) then
							TriggerEvent("Notify","sucesso","Aguarde o cliente no local",8000)
							...
							local distancia = GetDistanceBetweenCoords(restaurantes[destino].x, restaurantes[destino].y, restaurantes[destino].z, entregas[destinoEntrega].x,entregas[destinoEntrega].y,entregas[destinoEntrega].z,true)
							local pagamento = delivery.pagar(distancia) --#CALCULA E REALIZA PAGAMENTO delivery
							...
							-- TriggerEvent("Notify", "sucesso", "Você recebeu $" .. formatarNumero((parseInt(pagamento))) )
...
RegisterNetEvent('zDelivery:StartJob')
AddEventHandler('zDelivery:StartJob', function()
	AtualizaHUD(0,0,"Restaurante")
	local ped = PlayerPedId()
	if not trabalhando and not motodelivery then
		qtEntrega= 0
		dinheiroGanhoEntrega = 0
		AtualizaHUD(0,0,"Restaurante")
		trabalhando = true          
		pedido = true
		TriggerEvent("Notify", "sucesso", "Você iniciou o expediente, pegue a moto na frente do restaurante.",8000)
		openGui()
		destino = math.random(1,15)		
		distanciaDestino = calculaDistancia(restaurantes[destino].x,restaurantes[destino].y,restaurantes[destino].z)
		while distanciaDestino > 500 do
	...
	elseif trabalhando then
		...
		motodelivery = false
		RemoveBlip(blip)
		TriggerEvent("Notify", "importante", "Você encerrou seu expediente, obrigado por trabalhar para o delivery",8000)
	end
end)
