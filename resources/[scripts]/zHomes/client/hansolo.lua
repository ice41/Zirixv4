...
zCLIENT = {}
Tunnel.bindInterface('zHomes', zCLIENT)
zSERVER = Tunnel.getInterface('zHomes')
...
RegisterCommand(config.CommandLockDoor, function(source, args)
...
RegisterCommand(config.CommandEnterHome, function(source, args)
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	for k, v in pairs(homesList) do
		local distance = #(coords - vector3(v.x, v.y, v.z))
		if distance <= 1.5 and zSERVER.checkPermissions(k) then
			removeObjectHomes()
			DoScreenFadeOut(1000)
			houseOpen = tostring(k)
			zSERVER.setNetwork(tostring(k))
			zSERVER.applyHouseOpen(tostring(k))
			TriggerEvent('sounds:source', 'enterhouse', 0.7)
			Citizen.Wait(1000)

			TriggerEvent('zInventory:houseOpen',houseOpen)

			if v.type == 'Hotel' then
				createHotel(ped, v.x, v.y, 1500.0)
			end

			if v.type == 'Middle' then
				createMiddle(ped, v.x, v.y, 1500.0)
			end
			...
			if v.type == 'Hotel' then
				SetEntityCoords(ped, v.x-1.65, v.y-4.02, 1501.0)
				table.insert(internHouses, { v.x-1.65, v.y-4.02, 1501.0, 'exit', 'SAIR' })
			end

			if v.type == 'Middle' then
				SetEntityCoords(ped, v.x+3.63, v.y-15.43, 1502.0)
				table.insert(internHouses, { v.x+3.63, v.y-15.43, 1502.3, 'exit', 'SAIR' })
			end
...
function zCLIENT.updateHomesTheft(status)
	homesTheft = status
end
...
function zCLIENT.checkHomesTheft()
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	for k, v in pairs(homesTheft) do
		local distance = #(coords - vector3(v[1], v[2], v[3]))
		if distance <= 1.5 and zSERVER.checkHomeTheft(k) then
			theftPlayers = {}
			theftHomesX = v[1]
			theftHomesY = v[2]
...
function zCLIENT.enterHomesTheft(homeName)
	local ped = PlayerPedId()
	if not IsPedInAnyVehicle(ped) then
		...
		zSERVER.setNetwork(tostring(homeName))
		zSERVER.applyHouseOpen(tostring(homeName))
		TriggerEvent('sounds:source', 'enterhouse', 0.7)
...
function zCLIENT.getHomeStatistics()
	return tostring(houseOpen)
end

function zCLIENT.cleanupHomes()
	houseOpen = ''
end

function zCLIENT.getHomesRejoin()
	if houseOpen ~= '' and not theftOpen then
		zSERVER.removeNetwork(tostring(houseOpen))
		return true
	end
	return false
end

function zCLIENT.updateHomes(status)
	homesList = status
end
...
AddEventHandler('homes:togglePropertys', function(source, args)
	...
	if casas then
		...
		for k, v in pairs(homesList) do
			blips[k] = AddBlipForRadius(v.x, v.y, v.z, 5.0)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k, v in pairs(homesList) do
            ...
            if distance <= 1.5 then
                timeDistance = 5
                DrawText3Ds(v.x, v.y, v.z, '~w~/~p~'..config.CommandEnterHome..'    ~w~|    /~p~'..config.CommandLockDoor..'')
            end
        end
...
Citizen.CreateThread(function()
	while true do
		if houseOpen ~= '' then
			houseNetwork = zSERVER.getNetwork(tostring(houseOpen))
		end
		Citizen.Wait(1000)
	end
end)
	...
	while true do
		...
		if not IsPedInAnyVehicle(ped) and houseOpen ~= '' then
			...
			for k, v in pairs(internHouses) do
				...
				if distance <= 1.5 then
					timeDistance = 1
					DrawText3Ds(v[1], v[2], v[3], '~p~E~w~  '..v[5])
					...
					if IsControlJustPressed(1, 38) then
						if v[4] == 'exit' then
							...
							SetEntityCoords(ped, homesList[houseOpen].x, homesList[houseOpen].y, homesList[houseOpen].z + 0.1)
						...
						elseif v[4] == 'vault' then
							if zSERVER.checkIntPermissions(tostring(houseOpen)) then
