...
z = {}
Tunnel.bindInterface('zPlayer', z)
zCLIENT = Tunnel.getInterface('zPlayer')
...
RegisterCommand('call', function(source, args, rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if not zCLIENT.getHandcuff(source) then
...
RegisterCommand('pr', function(source, args, rawCommand)
	...
	if user_id and args[1] then
		if vRP.hasPermission(user_id, 'Police') then
			if vRPclient.getHealth(source) > 101 and not zCLIENT.getHandcuff(source) then
				local identity = vRP.getUserIdentity(user_id)
...
RegisterCommand('extras', function(source, args, rawCommand)
	...
	if user_id and args[1] then
		if vRP.hasPermission(user_id, 'Police') and vRP.hasPermission('Paramedic') then
			if vRPclient.getHealth(source) > 101 and not zCLIENT.getHandcuff(source) then
				zCLIENT.extraVehicle(args[1], source)
			end
		end
	end
end)
	...
	if user_id then
		if vRP.hasPermission(user_id, 'Police') then
			if vRPclient.getHealth(source) > 101 and not zCLIENT.getHandcuff(source) then
				...
				if vehicle then
					local plateUser = vRP.getVehiclePlate(vehPlate)
					local inVehicle = vRP.query('vRP/get_vehicles', { plate = vehPlate, vehicle = vehName })
					if inVehicle[1] then
						if inVehicle[1].arrest <= 0 then
							vRP.execute('vRP/set_arrest', { plate = vehPlate, vehicle = vehName, arrested = 1, arrested_time = parseInt(os.time()) })
...
RegisterCommand('service', function(source, args, rawCommand)
	...
	if user_id then
		if not zCLIENT.getHandcuff(source) then
			local distanceService, service = zCLIENT.distanceService(source)
			if distanceService then
				if service == 'Police' then
					...
					elseif vRP.hasPermission(user_id, 'waitPolice') then
						vRP.insertPermission(user_id, 'Police')
				...
				if service == 'Paramedic' then
					...
					elseif vRP.hasPermission(user_id, 'waitParamedic') then
						vRP.insertPermission(user_id, 'Paramedic')
				...
				if service == 'Mechanic' then
					...
					elseif vRP.hasPermission(user_id, 'waitMechanic') then
						vRP.insertPermission(user_id, 'Mechanic')
						TriggerClientEvent('tencode:StatusService', source, true)
	...
	if user_id then
		if parseInt(args[1]) >= 0 and parseInt(args[1]) <= 7 then
			if vRP.getPremium(user_id) then
				zCLIENT.weColors(source, parseInt(args[1]))
			end
		end
	end
end)
	...
	if user_id then
		if not zCLIENT.getHandcuff(source) then
			if args[2] == 'friend' then
				...
				if nplayer then
					if vRPclient.getHealth(nplayer) > 101 and not zCLIENT.getHandcuff(nplayer) then
...
RegisterCommand('rv', function(source, args, rawCommand)
	...
	if user_id then
		if vRP.hasPermission(user_id, 'Police') or vRP.hasPermission(user_id, 'Paramedic') or vRP.getInventoryItemAmount(user_id, 'rope') >= 1 then
			if vRPclient.getHealth(source) > 101 and not zCLIENT.getHandcuff(source) and not vRPclient.inVehicle(source) then
				local vehicle, vehNet, vehPlate, vehName, vehLock = vRPclient.vehList(source, 11)
				if vehicle then
					if vehLock ~= 1 then
						local nplayer = vRPclient.nearestPlayer(source, 11)
						if nplayer then
							zCLIENT.removeVehicle(nplayer)
						end
					end
				end
			end
			...
			if vRPclient.getHealth(source) > 101 and not zCLIENT.getHandcuff(source) and not vRPclient.inVehicle(source) then
				...
				if vehicle then
					if vehLock ~= 1 then
						local nplayer = vRPclient.nearestPlayer(source, 2)
						if nplayer then
							zCLIENT.putVehicle(nplayer, args[1])
						end
					end
				end
			end
...
RegisterCommand('andar', function(source, args, rawCommand)
	if args[1] then
		if not zCLIENT.getHandcuff(source) then
			zCLIENT.movementClip(source, walking[parseInt(args[1])][1])
		end
	end
end)
...
RegisterCommand('extras', function(source, args, rawCommand)
	...
	if user_id then
		if (vRP.hasPermission(user_id, 'Police') or vRP.hasPermission(user_id, 'Paramedic')) and parseInt(args[1]) > 0 then
			zCLIENT.toggleLivery(source, parseInt(args[1]))
		end
	end
end)
...
RegisterCommand('checktrunk', function(source, args, rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRPclient.getHealth(source) > 101 and not vRPclient.inVehicle(source) and not zCLIENT.getHandcuff(source) then
			local nplayer = vRPclient.nearestPlayer(source, 2)
			if nplayer then
...
function z.cuffToggle()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.hasPermission(user_id, 'Police') then
			if vRPclient.getHealth(source) > 101 and not zCLIENT.getHandcuff(source) and poCuff[user_id] == nil then
				if not vRPclient.inVehicle(source) then
					local nplayer = vRPclient.nearestPlayer(source, 2)
					if nplayer then
						if zCLIENT.getHandcuff(nplayer) then
							zCLIENT.toggleHandcuff(nplayer)
...
AddEventHandler('player:outfitFunctions', function(outfitFunctions)
	...
	if user_id then
	    if vRPclient.getHealth(source) > 101 and not zCLIENT.getHandcuff(source) then
			zCLIENT.setRemoveoutfit(source)
	    end
	end
end)
