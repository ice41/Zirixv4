...
emP = {}
Tunnel.bindInterface("zBennys",emP)

function emP.checkPermission()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		return (vRP.hasPermission(user_id,Config.bennyspermission))
	end
end
...
AddEventHandler("LSC:buttonSelected", function(name,button)
	...
	if user_id then
		if button.price then
			if vRP.tryPayment(user_id,button.price) or vRP.tryGetInventoryItem(user_id, 'dinheiro', button.price) then
...
AddEventHandler("LSC:finished", function(veh)
	...
	if user_id then
		...
		if vname and placa then
			local puser_id = nil
			local datap = vRP.query('vRP/get_vehicle_by_plate', {plate = placa})
			for _, v in pairs(datap) do 
				puser_id = v.user_id
			end
			...
			if puser_id then
				...
				vRP.setSData("custom:u"..parseInt(puser_id).."veh_"..vname.."_placa_"..placa,json.encode(custom))
			end
		end
	end
end)