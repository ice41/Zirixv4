...
z = {}
Tunnel.bindInterface('zHud', z)
zSERVER = Tunnel.getInterface('zHud')
...
RegisterNetEvent('zHud:changeHudOnOff')
AddEventHandler('zHud:changeHudOnOff',function(status)
	showHud = status
	SendNUIMessage({ display = status})
end)

RegisterCommand('hud', function(source, args)
	TriggerEvent('zHud:chgangeStatusHud')
end)

RegisterNetEvent('zHud:chgangeStatusHud')
AddEventHandler('zHud:chgangeStatusHud',function()
	if showHud then
		showHud = false
...
RegisterNetEvent('zHud:statusHud')
AddEventHandler('zHud:statusHud',function(status)
	showHud = status
end)


RegisterNetEvent('zHud:statusPhone')
AddEventHandler('zHud:statusPhone',function(status)
	openPhone = status
end)

RegisterNetEvent('zHud:statusGps')
AddEventHandler('zHud:statusGps',function(status)
	openGps = status
end)
...
RegisterNetEvent('zHud:voiceMode')
AddEventHandler('zHud:voiceMode', function(number)
	if number == 1 then
		voiceMode = 'Normal'
...
RegisterNetEvent('zHud:radioMode')
AddEventHandler('zHud:radioMode',function(number)
...
RegisterNetEvent('zHud:belt')
AddEventHandler('zHud:belt',function()
  local ped = PlayerPedId()
  local car = GetVehiclePedIsIn(ped)
	if car ~= 0 and (ExNoCarro or IsCar(car)) then
		TriggerEvent('cancelando',true)
		if seatbelt then
			TriggerEvent('zSounds:source','unbelt', 0.5)
			SetTimeout(2000,function()
				seatbelt = false
				TriggerEvent('cancelando',false)
			end)
		else
			TriggerEvent('zSounds:source','belt', 0.5)
    	...
    	if IsPedInAnyVehicle(ped) then
     		local vehicle = GetVehiclePedIsUsing(ped)
      		...
      		if engine and not IsPedOnAnyBike(ped) and not IsPedInAnyHeli(ped) and not IsPedInAnyPlane(ped) then
				...
				TriggerEvent('zSounds:source','sem-cinto', 0.5)
				end
      		end
    	end
	end
