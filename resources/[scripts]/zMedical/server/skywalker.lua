...
zSERVER = {}
Tunnel.bindInterface('zMedical',zSERVER)
zCLIENT = Tunnel.getInterface('zMedical')
		...
		if nplayer then
			if vRPclient.isInComa(nplayer) then
				...
				SetTimeout(30000,function()				
					...
					zCLIENT.zDiagnosticResetBleeding(nplayer)
...
RegisterCommand('laudo',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,config.laudopermission) then
		local source = source
		local user_id = vRP.getUserId(source)
...
function zSERVER.checkPermission(permission)
	local src = source
...
function zSERVER.checkServices()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
...
function zSERVER.raiox()
	...
	local foot = false
	if nplayer then
		local diagnostic = zCLIENT.getDiagnostic(nplayer)
