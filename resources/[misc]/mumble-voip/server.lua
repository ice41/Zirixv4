...
function SetPlayerRadioName(serverId, name)
	...
	if identity then
		if voiceData[serverId] then
			voiceData[serverId].radioName = identity.name
			TriggerClientEvent('mumble:SetVoiceData', -1, serverId, 'radioName', identity.name)
		end
	end
end

function GetPlayerRadioName()
	local source = source
	local user_id = vRP.getUserId(source)
	local identity = vRP.getUserIdentity(user_id)
	if identity then
...
exports('GetPlayerRadioName', GetPlayerRadioName)