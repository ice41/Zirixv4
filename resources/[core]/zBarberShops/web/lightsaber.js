$(document).ready(function () {
	...
	document.onkeyup = data => {
		const key = data.key;
		if (key === 'Escape') {
			$.post('https://zBarberShops/closeInterface', JSON.stringify({}))
		}
	};
	...
	$('input').change(function () {
		$.post('https://zBarberShops/updateSkin', JSON.stringify({
			value: false,
	...
	document.onkeydown = function (data) {
		if (data.which == 65) {
			$.post('https://zBarberShops/rotate', JSON.stringify("right"));
		}
		if (data.which == 68) {
			$.post('https://zBarberShops/rotate', JSON.stringify("left"));
		}
	}
});