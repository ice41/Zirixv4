...
zCLIENT = {}
Tunnel.bindInterface('zMultiChar', zCLIENT)
zSERVER = Tunnel.getInterface('zMultiChar')
...
RegisterNUICallback('spawnChar', function(data, cb)
    TriggerServerEvent('zMultiChar:teste', data.id)
end)

RegisterNUICallback('joinInTheCity', function(data, cb)
    TriggerServerEvent('zMultiChar:charChosen', tonumber(data.id))
    TriggerEvent('closeInterface')
end)

RegisterNUICallback('newChar', function(data, cb)
    TriggerServerEvent('zMultiChar:createChar')
end)
...
function TaskUpdateClothingOptions(model, data)
	SetPedComponentVariation(model, 04, data["pants"].item, data["pants"].texture, 02)
	SetPedComponentVariation(model, 03, data["arms"].item, data["arms"].texture, 02)
	SetPedComponentVariation(model, 08, data["t-shirt"].item, data["t-shirt"].texture, 02)
	SetPedComponentVariation(model, 09, data["vest"].item, data["vest"].texture, 02)
	SetPedComponentVariation(model, 011, data["torso2"].item, data["torso2"].texture, 02)
	SetPedComponentVariation(model, 06, data["shoes"].item, data["shoes"].texture, 02)
	SetPedComponentVariation(model, 01, data["mask"].item, data["mask"].texture, 02)
	SetPedComponentVariation(model, 010, data["decals"].item, data["decals"].texture, 02)
	SetPedComponentVariation(model, 07, data["accessory"].item, data["accessory"].texture, 02)
	SetPedComponentVariation(model, 05, data["bag"].item, data["bag"].texture, 02)
	if data["hat"].item ~= -1 and data["hat"].item ~= 0 then
		SetPedPropIndex(model, 00, data["hat"].item, data["hat"].texture, 02)
	else
		ClearPedProp(model, 00)
	end
	if data["glass"].item ~= -1 and data["glass"].item ~= 0 then
		SetPedPropIndex(model, 01, data["glass"].item, data["glass"].texture, 02)
	else
		ClearPedProp(model, 01)
	end
	if data["ear"].item ~= -1 and data["ear"].item ~= 0 then
		SetPedPropIndex(model, 02, data["ear"].item, data["ear"].texture, 02)
	else
		ClearPedProp(model, 02)
	end
	if data["watch"].item ~= -1 and data["watch"].item ~= 0 then
		SetPedPropIndex(model, 06, data["watch"].item, data["watch"].texture, 02)
	else
		ClearPedProp(model, 06)
	end
	if data["bracelet"].item ~= -1 and data["bracelet"].item ~= 0 then
		SetPedPropIndex(model, 07, data["bracelet"].item, data["bracelet"].texture, 02)
	else
		ClearPedProp(model, 07)
	end
end

RegisterNetEvent('zMultiChar:setupChar')
AddEventHandler('zMultiChar:setupChar', function()
    DoScreenFadeOut(0)
    DisplayRadar(false)
...
RegisterCommand('mult',function(source)
    TriggerEvent('zMultiChar:setupChar')
end)