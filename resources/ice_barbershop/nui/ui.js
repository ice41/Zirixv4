...
$(document).ready(function() {
    document.onkeydown = function(data) {
        if (data.keyCode == 27) {
            ...
            $('#cores').slideUp(0);
            $('#totalDireita').html('0'); 
            $.post('http://ice_barbershop/reset', JSON.stringify({}));
        }
    }
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.openBarberShop) {
            ...
            $.post('http://ice_barbershop/updateGui', JSON.stringify({part: dataPart}));  
        }        
        ...
        if(item.updateBarberShop){
            ...
            for (var i = 0; i <= item.drawable; i++) {    
                console.log (item.prefix); // Masculino ou feminino 
                console.log(dataPart);            
                $("#listagem").append(`
                    <div class="lista" data-id="${i}" onclick="select(this)" id="${dataPart}${i}" style="background-image: url('http://localhost/barber/${dataPart}/${item.prefix}/${i}.jpg'); background-size: 160px 160px;">
                        <div class="valorItem">€500 </div>
...
function changeColor(color, type){
    $.post('http://ice_barbershop/changeColor', JSON.stringify({ dataType: dataPart, color: color, type: type }));
}
...
function select(element) {
    ...
    $.post('http://ice_barbershop/changeCustom', JSON.stringify({ type: dataPart, id: partId }));    
}
...
$("#esquerda").click(function() {
    $.post('http://ice_barbershop/leftHeading', JSON.stringify({ value: 10 }));
})

$("#direita").click(function() {
    $.post('http://ice_barbershop/rightHeading', JSON.stringify({ value: 10 }));
})
...
$("#cart").click(function() {
    $("#container").fadeOut();
    $('#cores').slideUp(0);
    $.post('http://ice_barbershop/payament', JSON.stringify({ price: $('#totalDireita').text(), language: oldPart }));
    $('#totalDireita').html('0');
})