    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.action == 'setPrice') {
            if (item.typeaction == "add") {
                $('#totalDireita').html(`R$ ${item.price}`)
            }
        ...
        if(item.updateBarberShop){
            ...
            // | Alterar o Ip abaixo para o das suas imagens [1]
            $('#listagem').html('');
...
$("#cart").click(function() {
    $("#container").fadeOut();
    $('#cores').slideUp(0);
    $.post('http://mpr_barbershop/payament', JSON.stringify({ price: $('#totalDireita').text().replace('R$',''), parts: oldPart }));
    $('#totalDireita').html('R$ 0');
})