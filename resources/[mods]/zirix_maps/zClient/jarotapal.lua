...
local centerHotel = vector3(-1199.2733154297,-189.95799255371,46.786312103271)
...
AddEventHandler('playerSpawned', function ()
	if not Ran then
		...
		while cpt < 300 do
			...
			local zmin = math.floor(z)+0.5
			local zmax = math.ceil(z)+0.5
			
			if Vdist(coords,centerHotel) < 75.0 then
				for k,v in pairs(stairHeight) do
					if v > zmin and v<zmax then
						currentStair = k
						founded = true
						timer = cpt
						cpt = 3001
		...
		if founded then
			for k,v in pairs(stairIpl[currentStair]) do
				RequestIpl(v)
			end
		end
